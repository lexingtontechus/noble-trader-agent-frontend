...
--
-- PREREQUISITE: Run 20260511_cron_jobs.sql (or 00000000000000_setup_pg_cron.sql)
-- first to set up pg_cron and pg_net extensions.
-- ============================================================
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    url := current_setting('app.noble_base_url', true) || '/api/evolution/rotate?secret=' || current_setting('app.noble_cron_secret', true),
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := current_setting('app.noble_base_url', true) || '/api/evolution/optimize?secret=' || current_setting('app.noble_cron_secret', true),
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'x-cron-secret', current_setting('app.noble_cron_secret', true)
    ),
