...
function localWalkForwardValidation(prices, side) {
  ...
  for (let i = 1; i < testPrices.length; i++) {
    if (testPrices[i - 1] > 0) returns.push(Math.log(testPrices[i] / testPrices[i - 1]));
  }
  ...
  const totalReturn = testPrices.length > 1 ? (testPrices[testPrices.length - 1] - testPrices[0]) / testPrices[0] : 0;
...
function computeValidationScore(result, side, source = "fastapi") {
  ...
  const ddScore = Math.min(Math.max(1 - maxDd / 0.40, 0), 1);       // maxDD 0-40% → 1-0
