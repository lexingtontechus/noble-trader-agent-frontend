...
function extractSymbols(orders = []) {
  ...
  return Object.entries(symbolMap).map(([symbol, info]) => {
    ...
    if (netQty > 0) {
      ...
      avgPrice =
        (info.totalBuyValue + info.totalSellValue) /
        (info.totalBuyQty + info.totalSellQty);
      netSide = "flat";
    }
      ...
      {!isExpanded && (
        <div className="flex flex-wrap gap-2">
          {symbols.map((s) => (
            <div
              key={s.symbol}
              className="badge badge-lg badge-outline gap-1 py-3 px-4"
            >
              <span className="font-mono font-bold">{s.symbol}</span>
              <span
                className={`badge badge-xs ${s.positionInfo.netSide === "long" ? "badge-success" : s.positionInfo.netSide === "short" ? "badge-error" : "badge-ghost"}`}
              >
                {s.positionInfo.netSide === "long"
                  ? "L"
                  : s.positionInfo.netSide === "short"
                    ? "S"
                    : "—"}
              </span>
              {s.positionInfo.totalQty > 0 && (
                <span className="text-xs opacity-60">
                  {s.positionInfo.totalQty} shares
                </span>
              )}
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {symbols.map((s) => {
            ...
            const dataWithPosition = data
              ? { ...data, positionInfo: s.positionInfo }
              : null;
