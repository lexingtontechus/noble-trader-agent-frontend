...
/**
 * PriceFanChart — Renders a Monte Carlo price fan with percentile bands.
 *
 * Shows p5-p95 (outer), p25-p75 (inner), and median line.
 * Colors: green bands if pct_paths_positive > 0.5, red otherwise.
 */
...
function FanTooltip({ active, payload, medianStroke }) {
  ...
  return (
    <div className="bg-base-200 border border-base-300 rounded-lg p-2 shadow-lg text-xs">
      ...
      {d.p75 != null && (
        <p className="text-warning">P75: ${d.p75.toFixed(2)}</p>
      )}
      <p className="font-bold" style={{ color: medianStroke }}>
...
export default function PriceFanChart({ simulation, currentPrice = null }) {
  ...
  const { price_p5, price_p25, price_median, price_p75, price_p95 } =
    simulation;

  // Build chart data — step index as X axis
  ...
  // Color scheme based on expected direction
  ...
  return (
    <div style={{ minHeight: "200px", width: "100%" }}>
      <ResponsiveContainer width="100%" height={280}>
        ...
        >
          ...
          <Area
            ...
            dataKey="p95"
            stroke="none"
            fill={`rgba(${isBullish ? "34,197,94" : "239,68,68"},0.06)`}
            dot={false}
            strokeWidth={0}
          />
          ...
          <Area
            type="monotone"
            dataKey="p75"
            stroke={`rgba(${isBullish ? "34,197,94" : "239,68,68"},0.3)`}
            strokeWidth={1}
            fill={`rgba(${isBullish ? "34,197,94" : "239,68,68"},0.12)`}
            dot={false}
          />
          ...
          {/* P5 lower boundary — covers area below p25 to create inner band effect */}
          <Area
            type="monotone"
            dataKey="p5"
            stroke="none"
      ...
      <div className="flex flex-wrap gap-3 text-xs text-base-content/50 mt-1 justify-center">
        ...
        <span className="flex items-center gap-1">
          <span
            className="inline-block w-3 h-2 rounded-sm opacity-20"
            style={{ background: bandColor }}
          />
          P5–P95
        </span>
