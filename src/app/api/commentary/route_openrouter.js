...
export async function POST(request) {
  try {
    ...
    // Cache commentary by symbol + regime label — same regime = same commentary for 5 min
    const cacheKey = `commentary:${symbol}:${regime?.regime_label || "unknown"}`;
...
Provide specific, actionable insights based on the regime state and risk metrics.`;
    ...
    const responseData = { commentary };
    setCache(cacheKey, responseData, 5 * 60 * 1000); // 5 min cache
